...
$('.getStibee').on("click",function () {
	$('.loading-bar-wrap').removeClass("hidden");
	$.ajax({
		url: base_url+"/console/getStibee",
		data:$('#defaultForm').serialize(),
		dataType: "json",
		success: function (data) {
...
//구독자 추가
$('.addBtnSubscribers').on("click",function () {
	$('.loading-bar-wrap').removeClass("hidden");
	var listId= $('input[name=listId]').val()
	$.ajax({
		type:"POST",
		url: base_url+"/console/addsubscribers",
		data:$('#addFormSubscribers').serialize()+"&listId="+listId,
		dataType: "json",
		success: function (data) {
			if(data.alerts_status=="success") {
				// callToast("요청이 정상 처리되었습니다.","success","알림")
				location.reload();
			}else{
				$.each(data.alerts_title, function (key, value) {
					$.toast({
...
//구독자 완전삭제
$('#deleteSubscribers').on("click",function () {
	$('.loading-bar-wrap').removeClass("hidden");
	var listId= $('input[name=listId]').val()
	$.ajax({
		type:"POST",
		url: base_url+"/console/deletesubscribers",
		data:$('#defaultForm').serialize()+"&listId="+listId,
		dataType: "json",
		success: function (data) {
			if(data.alerts_status=="success") {
				// callToast("요청이 정상 처리되었습니다.","success","알림")
...
$('#unSubscribe').on("click",function () {
	...
	$.ajax({
		type:"POST",
		url: base_url+"/console/unsubscribe",
		data:$('#defaultForm').serialize()+"&listId="+listId,
		dataType: "json",
		success: function (data) {
			if(data.alerts_status=="success") {
